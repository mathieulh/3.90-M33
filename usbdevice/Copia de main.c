...
static SceOff Common_IoLseek(PspIoDrvFileArg *arg, SceOff ofs, int whence)
{
	SceOff res = lflash_driver->funcs->IoLseek(arg, ofs, whence);
	if (res < 0)
	{
		res = ofs;
	}
...
static int Common_IoRead(PspIoDrvFileArg *arg, char *data, int len)
{
	int res = lflash_driver->funcs->IoRead(arg, data, len);
	if (res != len)
	{
		memset(data, 0, len);		
	}
...
static int Common_IoWrite(PspIoDrvFileArg *arg, const char *data, int len)
{
	...
	int res = lflash_driver->funcs->IoWrite(arg, data, len);
...
static int Common_IoIoctl(PspIoDrvFileArg *arg, unsigned int cmd, void *indata, int inlen, void *outdata, int outlen)
{
	...
	/*if (cmd == 0x0210D033 || cmd == 0x0210D032)
	{
		return 0;
	}*/
...
int pspUsbDeviceSetDevice(u32 unit, int ronly, int unassign_mask)
{
	...
	msstor_driver->funcs->IoRead = Common_IoRead;
	msstor_driver->funcs->IoLseek = Common_IoLseek;
...
int module_start(SceSize args, void *argp)
{
	...
	fd = sceIoOpen("ms0:/lf.bin", PSP_O_WRONLY | PSP_O_CREAT | PSP_O_TRUNC, 0777);

	for (i = 0; i < 0x200; i++)
	{
		memset(outdata, 0xDA, 0x200);
		ReadLogicalPage(lfatfs_struct, i, outdata);
		sceIoWrite(fd, outdata, 0x200);
	}
	...
	for (i = 0; i < 4; i++)
	{
		...
		if (fd >= 0)
		{
			...
			{
				device_sizes[i] = *(u32 *)&outdata[0x14] + 20;				
			}

			if (i == 0)
				WriteFile("ms0:/f0.bin", outdata, 0x44);
			else if (i == 1)
				WriteFile("ms0:/f1.bin", outdata, 0x44);
