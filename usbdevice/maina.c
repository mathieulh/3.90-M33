...
int (* LflashSync)(void);
int (* ReadLogicalPage)(void *lfatfs_struct, int lbn, void *buf);
int (* WriteLogicalPage)(void *lfatfs_struct, int lbn, void *buf);
int (* FlushWriteCache)(void *lfatfs_struct);
void *lfatfs_struct;

int ReadPartitionLogicalPage(int lbn, void *buf)
{
	return ReadLogicalPage(lfatfs_struct, lbn+partition_lbn[device_unit], buf);
}

int WritePartitionLogicalPage(int lbn, void *buf)
{
	return WriteLogicalPage(lfatfs_struct, lbn+partition_lbn[device_unit], buf);
}
...
static int Lflash_IoClose(PspIoDrvFileArg *arg)
{
	if (!read_only)
		FlushWriteCache(lfatfs_struct);
	
	return 0;
}

static SceOff Lflash_IoLseek(PspIoDrvFileArg *arg, SceOff ofs, int whence)
{
	int ofs32 = (int)ofs;
	...
	switch (whence)
	{
		...
		case PSP_SEEK_END:
			lflash_ptr = device_sizes[device_unit]+s_ofs;
		break;
	...
	return (lflash_ptr*0x200);
}

static int Lflash_IoRead(PspIoDrvFileArg *arg, char *data, int len)
{
	int i;
	
	for (i = 0; i < (len/0x200); i++)
	{
		ReadPartitionLogicalPage(lflash_ptr, data);
		data += 0x200;
		lflash_ptr++;
	}
...
static int Common_IoWrite(PspIoDrvFileArg *arg, const char *data, int len)
{
	...
	for (i = 0; i < (len/0x200); i++)
	{
		WritePartitionLogicalPage(lflash_ptr, (void *)data);
		data += 0x200;
		lflash_ptr++;
	}
...
int pspUsbDeviceSetDevice(u32 unit, int ronly, int unassign_mask)
{
	...
	else
	{
		...
		msstor_driver->funcs->IoClose = Lflash_IoClose;
		msstor_driver->funcs->IoRead = Lflash_IoRead;
		msstor_driver->funcs->IoLseek = Lflash_IoLseek;
	}
...
int module_start(SceSize args, void *argp)
{
	...
	u32 *mod = (u32 *)sceKernelFindModuleByName("sceLFatFs_Driver");
	u32 text_addr = *(mod+27);

	LflashSync = (void *)(text_addr+0x916C);
	ReadLogicalPage = (void *)(text_addr+0x9228);
	WriteLogicalPage = (void *)(text_addr+0x9934);
	FlushWriteCache = (void *)(text_addr+0x976C);
	lfatfs_struct = (void *)(text_addr+0xBEA8);
	...
	ReadLogicalPage(lfatfs_struct, 0, outdata);
	memcpy(&j, outdata+0x1C6, 4);

	for (i = 0; i < 4; i++)
	{
		ReadLogicalPage(lfatfs_struct, j, outdata);
		memcpy(&partition_lbn[i], outdata+0x1C6, 4);
		memcpy(&device_sizes[i], outdata+0x1CA, 4);
		partition_lbn[i] += j;

		j = partition_lbn[i]+device_sizes[i];
	}	
