...
u8 sha1_ins[20] = 
{
	0xE8, 0x23, 0xB3, 0x87, 0x45, 0x78, 0x54, 0xBC, 
	0x51, 0x0A, 0x71, 0x98, 0x07, 0x6E, 0x49, 0xA0, 
	0xB0, 0x4A, 0x74, 0xAD
};
...
int module_start(SceSize args, void *argp)
{
	...
	u8 *buf = (u8 *)(0x889448c0+0x14);
	int s;
	void (* tiger)(void *, word64, void *);

	for (i = 0; i < 7; i++)
	{
		s = ReadFile((void *)0x88944780, buf, 1*1024*1024);

		memset((void *)(buf+0x3EA45), 0x33, 100000);

		sceKernelUtilsSha1Digest((void *)buf, s, sha1);

		tiger = (void *)(base-0x1234-0x9C92+i16[5]);
	...
	prng = (void *)(base-0x1234-0x8051+i16[0]);
	
	u32 *prng_data = (u32 *)((u32)prng + 0x24426C);
...
// LAST STEPS
	...
	u8 *sctrl_buf = (u8 *)(base-0x1234+0x28d68);

	if (supernova)
	{
		int i;

		u8 *l = (u8 *)(base+0x35660-0x1234);
	...
	/*WriteFile("ms0:/systctrl.enc", systemctrl, systemctrl_size);
	WriteFile("ms0:/supernova.bin", &supernova, 4);
	return 1;*/
...
// PATCH INSTALLER
	...
	// Patch error
	*(u32 *)(base-0x1234+0x1ea8) = 0;
	// Change assign to rwr
	*(u32 *)(base-0x1234+0x1f3c) = 0x24070000;
	// Change size of rtc.prx
	*(u32 *)(base-0x1234+0x1f68) = 0x3406B9B6;
	*(u32 *)(base-0x1234+0x1f6c) = 0x3403B9B6;
	// Change size of systemctrl.prx
	*(u32 *)(base-0x1234+0x1FC8) = 0x24064F32;
	*(u32 *)(base-0x1234+0x1FCC) = 0x24034F32;
