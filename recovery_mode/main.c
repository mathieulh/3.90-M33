...
int LoadReboot()
{
	SceUID mod = vshKernelLoadModuleVSH("flash0:/kd/reboot150.prx", 0, NULL);

	if (mod < 0)
	{
		mod = vshKernelLoadModuleVSH("ms0:/seplugins/reboot150.prx", 0, NULL);
	}
...
void enableUsb(int device) 
{
	...
	if (device != 0)
	{
		...
		pspUsbDeviceSetDevice(unit, 0, 0);	
	}
	else
	{
		unit = -1;
...
char  hide[116], bootprog[116], noumd[116], region[116], daxupd[80], hidepics[100];
char patch[110], bootbin[110];
char plugins[21][50];
char vshspeed[128], umdspeed[128];
char game[128];
char freereg[128];
char hbreset[128];
char *plugins_p[21];
	...
	while(1) 
	{
		...
		else if(result == 1) 
		{
			...
			while(1) 
			{
				...
				sprintf(game, "Game folder homebrew (currently: %s)", config.gamekernel150 ? "1.50 Kernel" : "3.90 Kernel");
				...
				sprintf(daxupd, "Use M33 network update (currently %s)", config.notusedaxupd ? "Disabled" : "Enabled");
				sprintf(hidepics, "Hide PIC0.PNG and PIC1.PNG in game menu (currently %s)", config.hidepics ? "Enabled" : "Disabled");
				...
				{
					conitems[11] = daxupd;
					conitems[12] = hidepics;
					result = doMenu(conitems, 13, oldselection, conmsg, 2);
				}
				else
				{
					sprintf(usbcharge, "Charge battery when USB cable plugged (currently: %s)", config.usbcharge ? "Enabled" : "Disabled");
					conitems[11] = usbcharge;
					conitems[12] = daxupd;
					conitems[13] = hidepics;
					result = doMenu(conitems, 14, oldselection, conmsg, 2);
				}
				...
				else if (result == 3)
				{
					...
					printf(" > Game folder homebrew: %s", (config.gamekernel150) ? "1.50 Kernel" : "3.90 Kernel"); 
					delay(1000000); 
				}
				else if (result == 4)
				{
				...
				else if (model == PSP_MODEL_SLIM_AND_LITE && result == 11)
				{
					config.usbcharge = !config.usbcharge;
					printf(" > Charge battery when USB cable plugged: %s", (config.usbcharge) ? "Enabled" : "Disabled");
					delay(1000000); 
				}
				else if ((result == 12 && model == PSP_MODEL_SLIM_AND_LITE) || result == 11)
				{
					config.notusedaxupd = !config.notusedaxupd;
					printf(" > Use M33 network update: %s", !(config.notusedaxupd) ? "Enabled" : "Disabled");
					delay(1000000); 
				}
				else if (result == 13 || result == 12)
				{
					config.hidepics = !config.hidepics;
					printf(" > Hide PIC0.PNG and PIC1.PNG in game menu: %s", (config.hidepics) ? "Enabled" : "Disabled");
					delay(1000000); 
				}
		...
		else if (result == 2)
		{
			...
			if (vshKernelGetModel() == PSP_MODEL_STANDARD && config.gamekernel150)
			{
				LoadReboot();
			}

			vshKernelLoadExecVSHMs2(PROGRAM, &param);
		}
