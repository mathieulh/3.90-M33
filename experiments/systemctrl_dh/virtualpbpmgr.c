...
int virtualpbp_init()
{
	vpbps = (VirtualPbp *)malloc(MAX_FILES*sizeof(VirtualPbp));
...
int virtualpbp_add(char *isofile, ScePspDateTime *mtime, VirtualPbp *res)
{
	...
	vpbps[g_index].header[5] = offset; // UNKNOWN.PNG 
...
int virtualpbp_chstat(int i, SceIoStat *stat, int bits)
{
	...
	sceKernelSignalSema(vpsema, 1);
	return sceIoChstat(vpbps[i].isofile, stat, bits);
}
...
int virtualpbp_remove(int i)
{
	...
	sceKernelSignalSema(vpsema, 1);
	return sceIoRemove(vpbps[i].isofile);
}
