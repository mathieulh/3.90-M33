...
PSP_MODULE_INFO("M33PSAR_Creator", 0x1000, 1, 0);
...
int PatchPower(u8 *buf, int size, u8 *buf2, u8 *header)
{
	...
	return PackModule(buf2, nsize, buf, header, OE_TAG_KERNEL_352);
}
...
int PatchVshMain(u8 *buf, int size, u8 *buf2, u8 *header)
{
	...
	ClearCaches();
	return PackModule(buf2, nsize, buf, header, OE_TAG_USER_352);
}
...
int PatchGamePlugin(u8 *buf, int size, u8 *buf2, u8 *header)
{
	...
	_sw(0x1000fff6, text_addr+0xFDD8);
	_sw(0x24040000, text_addr+0xFDDC);
...
#define N_EXCEPTIONS_150	3
#define N_EXCEPTIONS_352	10

char *exceptions_150[N_EXCEPTIONS_150] =
{
	"flash0:/kd/pspbtcnf_game.txt",
	"flash0:/kd/pspbtcnf_updater.txt",
	"flash0:/kd/loadexec.prx"
...
char *exceptions_352[N_EXCEPTIONS_352] =
{
	...
	"flash0:/kd/sysmem.prx",
	"flash0:/vsh/module/vshmain.prx",
	"flash0:/vsh/module/msvideo_main_plugin.prx",
	"flash0:/vsh/module/game_plugin.prx"	
};

int IsException(char *name, int is150)
{
	int i, n = 0;
	char **p;
...
int extract_psar(u8 *dataPSAR, int cbFile, u8 *dataOut, u8 *dataOut2, int is150)
{
    ...
    if (cbOut <= 0)
    {
        ErrorExit("Failed to decode(1)\n");
        return -1;
    }
    if (cbOut != SIZE_A)
    {
        ErrorExit("Start PSAR chunk has invalid size (%d)\n", cbOut);
        return -1;
    }
    ...
    while (iBase < cbFile-OVERHEAD)
    {
        ...
        if (cbOut <= 0)
        {
            ErrorExit("Abort return cb=%d [@$%x]\n", cbOut, iBase);
            //break;
        }
        if (cbOut != SIZE_A)
        {
            ErrorExit("Bad size for filename %d\n", cbOut);
            //break;
        }
        ...
        if (cbExpanded > 0)
        {
            ...
            if (cbOut > 10 && dataOut[0] == 0x78 && dataOut[1] == 0x9C)
            {
				...
				{
					memcpy(dataOut2, dataOut, cbOut);
					ret = cbExpanded = cbOut;					
				}                
                    ...
                    if (is150)
					{
						for (i = 0; i < 112; i++)
						{
							if (strcmp(name, subset150[i]) == 0)
							{
								...
								if (psarAddFile(name, dataOut2, cbExpanded, dataOut, 2000000, 0, origsize) < 0)
								{
									ErrorExit("Error writing file %s\n.\n", name);
								}	
						...
						if (strcmp(name, "flash0:/kd/syscon.prx") == 0)
						{
							...
							if (sz < 0)
								ErrorExit("Cannot decrypt loadexec.\n");
					...
					else 
					{
						...
						if (addit)
						{
							...
							if (strncmp(name, "ipl", 3) != 0)
							{
								...
								if (psarAddFile(name, dataOut2, cbExpanded, dataOut, 2000000, sigcheck, origsize) < 0)
								{
									ErrorExit("Error writing file %s\n.", name);
								}
								...
								{
									if (psarAddFile("flash0:/font/kr0.pgf", dataOut2, cbExpanded, dataOut, 2000000, sigcheck, origsize) < 0)
									{
										ErrorExit("Error writing file %s\n.\n", "flash0:/font/kr0.pgf");
									}
								}
							...
							if (strcmp(name, "flash0:/kn/loadexec.prx") == 0)
							{
								...
								if (sz < 0)
									ErrorExit("Can't decrypt loadexec.\n");
							...
							else if (strcmp(name, "flash0:/kn/sysmem.prx") == 0)
							{
								...
								if (sz < 0)
									ErrorExit("Cannot decrypt sysmem.prx.\n");
                ...
                {
                    ErrorExit("ERROR: deflate error $%x\n", ret);
                    //nErr++;
                }
            }
            else
            {
                ErrorExit("decode data ERROR cbOut=%d\n", cbOut);
                //nErr++;
            }			
        }
        ...
        {
            ErrorExit("cbExpanded bogus (%d) -- abort\n", cbExpanded);
            //break;
        }
	...
	if (sz <= 0)
	{
		ErrorExit("Cannot decrypt reboot.bin.\n");
	}
	...
	if (is150)
	{
		for (i = 0; i < sizeof(reboot150); i++)
		{
			if (memcmp(reboot150+i, "reboot.bin", 10) == 0)
			{
				...
				if (psarAddFile("flash0:/kn/reboot150.prx", reboot150, sizeof(reboot150), dataOut2, 2000000, 0, 0) < 0)
				{
					ErrorExit("Error writing file %s\n", "flash0:/kn/reboot150.prx");
				}
		...
		if (psarAddFile("flash0:/kd/uart4.prx", uart4, sizeof(uart4), dataOut2, 2000000, 0, 0) < 0)
		{
			ErrorExit("Error writing file %s\n", "flash0:/kd/uart4.prx");
		}

		if (psarAddFile("flash0:/kd/recovery.prx", recovery, sizeof(recovery), dataOut2, 2000000, 0, 0) < 0)
		{
			ErrorExit("Error writing file %s\n", "flash0:/kd/recovery.prx");
		}
	}
	...
	else
	{
		...
		if (compsize <= 0)
		{
			ErrorExit("Compression error.\n");
		}

		//WriteFile("ms0:/reboot352.def", dataOut, compsize);
		...
		for (i = 0; i < sizeof(systemctrl150); i++)
		{
			if (memcmp(systemctrl150+i, "reboot.bin", 10) == 0)
			{
				...
				if (psarAddFile("flash0:/kd/rtc.prx", systemctrl150, sizeof(systemctrl150), dataOut2, 2000000, 0, 0) < 0)
				{
					ErrorExit("Error writing file %s\n", "flash0:/kd/rtc.prx");
				}
		...
		if (psarAddFile("flash0:/kn/idcanager.prx", idcanager, sizeof(idcanager), dataOut2, 2000000, 0, 0) < 0)
		{
			ErrorExit("Error writing file %s\n", "flash0:/kn/idcanager.prx");
		}

		if (psarAddFile("flash0:/kn/popcorn.prx", popcorn, sizeof(popcorn), dataOut2, 2000000, 0, 0) < 0)
		{
			ErrorExit("Error writing file %s\n", "flash0:/kn/popcorn.prx");
		}

		if (psarAddFile("flash0:/kn/systemctrl.prx", systemctrl352, sizeof(systemctrl352), dataOut2, 2000000, 0, 0) < 0)
		{
			ErrorExit("Error writing file %s.\n", "flash0:/kn/systemctrl.prx");
		}

		if (psarAddFile("flash0:/kn/vshctrl.prx", vshctrl, sizeof(vshctrl), dataOut2, 2000000, 0, 0) < 0)
		{
			ErrorExit("Error writing file %s\n.\n", "flash:/kn/vshctrl.prx");
		}

		if (psarAddFile("flash0:/kn/march33.prx", march33, sizeof(march33), dataOut2, 2000000, 0, 0) < 0)
		{
			ErrorExit("Error writing file %s\n.\n", "flash:/kn/march33.prx");
		}

		if (psarAddFile("flash0:/kn/galaxy.prx", galaxy, sizeof(galaxy), dataOut2, 2000000, 0, 0) < 0)
		{
			ErrorExit("Error writing file %s\n.\n", "flash:/kn/galaxy.prx");
		}

		if (psarAddFile("flash0:/kn/pspbtknf.bin", pspbtknf_bin, sizeof(pspbtknf_bin), dataOut2, 2000000, 0, 0) < 0)
		{
			ErrorExit("Error writing file %s\n.\n", "flash:/kn/pspbtknf.bin");
		}

		if (psarAddFile("flash0:/kn/pspbtlnf.bin", pspbtlnf_bin, sizeof(pspbtlnf_bin), dataOut2, 2000000, 0, 0) < 0)
		{
			ErrorExit("Error writing file %s\n.\n", "flash:/kn/pspbtlnf.bin");
		}
	}	
...
u8 sha1_352[20] = 
{
	0xCE, 0x5D, 0xC9, 0x94, 0xDB, 0x36, 0x96, 0x8A, 
	0xCF, 0x2F, 0x48, 0x4D, 0x16, 0x13, 0x38, 0x65, 
	0x8F, 0xB8, 0x3E, 0xAF
};
...
int main(int argc, char *argv[])
{
	...
	printf("3.52 M33 FIRMWARE CREATOR\n\n");

	if (sceKernelFindModuleByName("SystemControl150"))
	{
		dir = "ms0:/PSP/GAME150/M33UPDATE_352";		
		sceIoMkdir("ms0:/PSP/GAME150", 0777);		
	}
	else
	{
		dir = "ms0:/PSP/GAME/M33UPDATE_352";
		sceIoMkdir("ms0:/PSP/GAME", 0777);
	}

	int res = sceIoMkdir(dir, 0777);
	if (res < 0)
	{
		if (res != 0x80010011)
		{
			ErrorExit("Cannot create directory %s.\n", dir);
		}
	}

	sprintf(path, "%s/EBOOT.PBP", dir);
	
	pbp = sceIoOpen(path, PSP_O_WRONLY | PSP_O_CREAT | PSP_O_TRUNC, 0777);
	if (pbp < 0)
	{
		ErrorExit("Cannot create file %s.\n", path);
	}

	memset(&header, 0, sizeof(header));
	sceIoWrite(pbp, &header, sizeof(header));

	header[0] = 0x50425000;
	header[1] = 0x10000;

	header[2] = sceIoLseek(pbp, 0, PSP_SEEK_CUR);
	sceIoWrite(pbp, sfo, sizeof(sfo));

	header[3] = sceIoLseek(pbp, 0, PSP_SEEK_CUR);
	sceIoWrite(pbp, icon0, sizeof(icon0));

	header[8] = header[7] = header[6] = header[5] = header[4] = sceIoLseek(pbp, 0, PSP_SEEK_CUR);
	sceIoWrite(pbp, elf, sizeof(elf));

	header[9] = sceIoLseek(pbp, 0, PSP_SEEK_CUR);
	sceIoLseek(pbp, 0, PSP_SEEK_SET);
	sceIoWrite(pbp, &header, sizeof(header));

	sceIoLseek(pbp, header[9], PSP_SEEK_SET);

	dataPSAR = (u8 *)memalign(0x40, PSAR_SIZE+64);

	if (!dataPSAR)
		ErrorExit("Mem alloc error.\n");
	...
	if (psarWriteBuf(version, sizeof(version)) < 0)
		ErrorExit("File write error.\n");
	...
	printf("Loading 1.50 firmware... ");

	fd = sceIoOpen("150.PBP", PSP_O_RDONLY, 0777);
	if (fd < 0)
	{
		ErrorExit("File %s couldn't be opened.\n", "150.PBP");
	}
	...
	{
		sceIoClose(fd);
		ErrorExit("PBP signature don't match.\n");
	}
	...
	if (size != 10149440)
	{
		ErrorExit("1.50 psar size doesn't match.\n");
		sceIoClose(fd);
	}

	if (sceIoRead(fd, dataPSAR, 10149440) != 10149440)
	{
		ErrorExit("Not enough data in file.\n");
		sceIoClose(fd);
	}
	...
	{
		ErrorExit("SHA1 of 1.50 psar doesn't match expected.\n");		
	}

	printf("OK.\n");

	printf("Writing 1.50 firmware... ");

	extract_psar(dataPSAR, 10149440, dataOut, dataOut2, 1);

	printf("OK.\n");

	printf("Loading 3.52 firmware... ");

	fd = sceIoOpen("352.PBP", PSP_O_RDONLY, 0777);
	if (fd < 0)
	{
		ErrorExit("File %s couldn't be opened.\n", "352.PBP");
	}
	...
	if (size != PSAR_SIZE)
	{
		ErrorExit("3.52 psar size doesn't match.\n");
		sceIoClose(fd);
	}

	if (sceIoRead(fd, dataPSAR, PSAR_SIZE) != PSAR_SIZE)
	{
		ErrorExit("Invalid 3.52 update file (2).\n");
		sceIoClose(fd);
	}
	...
	if (memcmp(sha1, sha1_352, 20) != 0)
	{
		ErrorExit("SHA1 of 3.52 psar doesn't match expected.\n");			
	}

	printf("OK.\n");
	printf("Writing 3.52 firmware... ");	

	extract_psar(dataPSAR, PSAR_SIZE, dataOut, dataOut2, 0);

	if (psarEnd(dataOut, 2000000) < 0)
	{
		ErrorExit("Error finishing file.\n");
	}
