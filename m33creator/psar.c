...
{
	if (size + mem_position >= (4*1024*1024))
	{
		if (Flush() < 0)
			return -1;
	}
...
int psarInit(SceUID descriptor)
{
	...
	sceKernelVolatileMemLock(0, (void *)&memaddr, &x);
...
int	psarAddFile(char *file, void *buf, int size, u8 *buf2, int buf2size, int sigcheck, int origsize)
{
	...
	if (WriteDelayed(&entry, sizeof(entry)) != sizeof(entry))
		return -1;

	if (WriteDelayed(p, entry.chunksize) != entry.chunksize)
	{
		return -1;		
	}
