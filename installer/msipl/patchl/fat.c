...
int MsFatMount()
{
	...
	//Kprintf("Payloadex Fat driver v 0.21.\n");
	//Kprintf("MsFatMount\n");
...
FILE_NOT_FOUND:
	...
	//Kprintf("File or dir \"%s\" not found.\n", filename);
	return -1;
}
...
int MsFatOpen(char *path)
{
	...
	while (1)
	{
		...
		*q = 0;
		...
		if ((*p == '/' || *p == '\\') && *(p+1) == 0)
		{
			//Kprintf("Specified path \"%s\" is not valid for a file.\n", path);
			return -1;
		}
