...
typedef struct PspMemoryBlockInfo
{
	SceSize size;
	u8 unk[0x24];
	u32 startaddr;
...
/**
 * Gets info about a memory block
 *
 * @param uid - The uid of the memory block
 * @param info - Pointer to the ::PspSysmemBlockInfo structure.
 *
 * @returns 0 on success
*/
 ...
 * @param ch - The chatacter used to fill the buffer
 * @param size - The size to fill
 *
 * @returns 0 on success
*/
int sceKernelMemset(void *buf, int ch, SceSize size);

/**
 * Fills a buffer with a word
 *
 * @param buf - The buffer to fill, aligned to 4-bytes boundary
 * @param w - The word used to fill the buffer
 * @param size - The size to fill
 *
 * @returns 0 on success
*/
int sceKernelMemset32(void *buf, int w, SceSize size);

/**
 * Copies a buffer to other one 
 *
 * @param out - Destiny buffer
 * @param in - Source buffer
 * @param size - Size to copy
 *
 * @returns 0 on success
*/
int sceKernelMemmove(void *out, void *in, int size);

/**
 * Copies a buffer to other one, and then fills the input with the specified character
 *
 * @param out - Destiny buffer
 * @param in - Source buffer
 * @param size - Size to copy
 * @param ch - The char to fill the input buffer after the copy
 *
 * @returns 0 on success
*/
int sceKernelMemmoveWithFill(void *out, void *in, int size, int ch);

/**
 * Set dnas data
 *
 * @param dnas - Unknown dnas data
*/
void sceKernelSetDNAS(void *dnas);
...
*/
void sceKernelSetSystemStatus(int status);

/**
 * Gets the memory info about an address
 *
 * @param addr - The address to query
 * @retSize - Pointer to a variable that receives the block size
 * @uid - Pointer to a variable that  receives the uid of the block to which addr belongs
 *
 * @returns < 0 on error
*/
int sceKernelQueryMemoryInfo(void *addr, SceSize *retSize, SceUID *uid);

/**
 * Fills the free blocks of a memory partition
 *
 * @param uid - The partition uid
 * @param fill - The character for fill
 *
 * @returns 0 on normal completion
 *
*/
int sceKernelFillFreeBlock(SceUID uid, int fill);

void SysMemForKernel_7B3E7441();
void SysMemForKernel_E0058030();
