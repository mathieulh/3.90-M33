...
u8 keyseed[16] __attribute__((aligned(4))) = 
{
	0x11, 0x8C, 0x73, 0x31, 0x8A, 0x2E, 0x00, 0xEF, 
	0x27, 0x96, 0x30, 0x5C, 0xA5, 0xCD, 0xEB, 0x3B
};
...
int sceKernelGzipDecompressPatched(u8 *dest, u32 destSize, const u8 *src, void *unknown)
{
	...
	{		
		...
		// _sw(0, 0x88600108);
		// keyseed[4] <-> 1F8DFD5D
		output = (u8 *)((u32)output ^ *(u32 *)&keyseed[4]);
		_sw(0, (u32)(output-0x17EDFE6D));
		_sw(0, (u32)(output-0x17EDFE55));
...
void PatchInit()
{
	...
	// = memcpy(keyseed, 0x883f0000, 0x10);
	memcpy(keyseed, (void *)((*(u32 *)&keyseed[0x0C] ^ 0xB3D4A884) - 0x6521), 0x10); 
}
