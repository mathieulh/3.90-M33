...
int main(void) {
	...
	char *advitems[] = { "Back", "Advanced configuration -> ", "Toggle USB (flash0-up)", "Toggle USB (flash1-up)", "Toggle USB (flash0-psp)", "Toggle USB (flash1-psp)" };
	...
	myDebugScreenSetBackColor(RGB(103, 114, 122));
	while(p) 
	{
		clearScreen();
		int result;
		...
		if(result == 3) 
		{
			...
			{
				clearScreen();
				result = doMenu(advitems, 6, 0, advmsg, 2, 7);
		...
		if(result == 1) 
		{
			...
			while(o) 
			{
				...
				sprintf(game, "Game folder homebrew (currently: %s)", config.gamekernel150 ? "1.50 Kernel" : "3.10 Kernel");
				...
				else if (result == 3)
				{
					...
					printf(" > Game folder homebrew: %s", (config.gamekernel150) ? "1.50 Kernel" : "3.10 Kernel"); 
					delay(1100000); 
				}
				else if (result == 4)
				{
