...
int MyRead(PspIoDrvFileArg *arg, char *data, int len)
{
	int res = IoRead(arg, data, len);
	Kprintf("MSSTOR read %d sectors.\n", len/512);
	return res;
}

SceOff (*IoLseek)(PspIoDrvFileArg *arg, SceOff ofs, int whence); 
SceOff MySeek(PspIoDrvFileArg *arg, SceOff ofs, int whence)
{
	SceOff res = IoLseek(arg, ofs, whence);

	SceOff x = ofs / 512;
	Kprintf("MSSTOR seek sector 0x%08X.\n", (u32)x);
	return res;
}

int (*IoRead2)(PspIoDrvFileArg *arg, char *data, int len); 
int MyRead2(PspIoDrvFileArg *arg, char *data, int len)
{
	int res = IoRead2(arg, data, len);
	Kprintf("MSSTOR_P1 read %d sectors.\n", len/512);
	return res;
}

SceOff (*IoLseek2)(PspIoDrvFileArg *arg, SceOff ofs, int whence); 
SceOff MySeek2(PspIoDrvFileArg *arg, SceOff ofs, int whence)
{
	SceOff res = IoLseek2(arg, ofs, whence);

	SceOff x = ofs / 512;
	Kprintf("MSSTOR_P1 seek sector 0x%08X.\n", (u32)x);
	return res;
}

int (*IoRead3)(PspIoDrvFileArg *arg, char *data, int len); 
int MyRead3(PspIoDrvFileArg *arg, char *data, int len)
{
	int res = IoRead3(arg, data, len);
	Kprintf("FATMS read %d\n", len);
	return res;
}
...
int MyDread(PspIoDrvFileArg *arg, SceIoDirent *dir)
{
	...
	if (res > 0 && dir->d_private)
	{
		if (g_i++ < 20)
		{
			sprintf(g_file, "ms0:/dread_%d.bin", g_i);
			WriteFile(g_file, dir->d_private, 0x414);
		}
	}
...
int module_start(SceSize args, void *argp)
{
	...
	Kprintf("My debug module.\n");*/

	PspIoDrv *driver = sctrlHENFindDriver("fatms");
	/*IoIoctl = driver->funcs->IoIoctl;
	driver->funcs->IoIoctl = MyIoctl;

	IoDevctl = driver->funcs->IoDevctl;
	driver->funcs->IoDevctl = MyDevctl;

	IoOpen = driver->funcs->IoOpen;
	driver->funcs->IoOpen = MyOpen;

	IoDopen = driver->funcs->IoDopen;
	driver->funcs->IoDopen = MyDopen;*/

	IoDread = driver->funcs->IoDread;
	driver->funcs->IoDread = MyDread;

	/*PspIoDrv *driver = sctrlHENFindDriver("msstor");
	IoRead = driver->funcs->IoRead;
	IoLseek = driver->funcs->IoLseek;
	driver->funcs->IoRead = MyRead;
	driver->funcs->IoLseek = MySeek;
	
	driver = sctrlHENFindDriver("msstor0p");
	IoRead2 = driver->funcs->IoRead;
	IoLseek2 = driver->funcs->IoLseek;
	driver->funcs->IoRead = MyRead2;
	driver->funcs->IoLseek = MySeek2;

	driver = sctrlHENFindDriver("fatms");
	IoRead3 = driver->funcs->IoRead;
	driver->funcs->IoRead = MyRead3;*/
