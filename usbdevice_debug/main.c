...
static int Lflash_IoRead(PspIoDrvFileArg *arg, char *data, int len)
{
	...
	Kprintf("Read len %08X res %08X.\n", len, res);
	return res;
}

SceOff Lflash_IoSeek(PspIoDrvFileArg *arg, SceOff ofs, int whence)
{
	SceOff res = lflash_driver->funcs->IoLseek(arg, ofs, whence);
	Kprintf("IoLseek result %08X.\n", (u32)res);
	return res;
}
...
int pspUsbDeviceSetDevice(u32 unit, int ronly, int unassign_mask)
{
	...
	else
	{
		...
		msstor_driver->funcs->IoRead = Lflash_IoRead;//lflash_driver->funcs->IoRead;
		msstor_driver->funcs->IoLseek = Lflash_IoSeek;//lflash_driver->funcs->IoLseek;
	}
