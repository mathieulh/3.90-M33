...
int main()
{
	...
	WriteFile("ms0:/_sctrl_.bin", &size, 4);

	/*size = ReadFile("ms0:/pspbtjnf.bin", buf, sizeof(buf));

	size = WriteFile("flash0:/kd/pspbtjnf.bin", buf, size);
	WriteFile("ms0:/_jnf_.bin", &size, 4);*/

	/*size = ReadFile("ms0:/usbdevice.prx", buf, sizeof(buf));
	size = WriteFile("flash0:/kd/usbdevice.prx", buf, size);
	WriteFile("ms0:/_usbdev_.bin", &size, 4);

	size = ReadFile("ms0:/recovery.prx", buf, sizeof(buf));
	size = WriteFile("flash0:/vsh/module/recovery.prx", buf, size);
	WriteFile("ms0:/_rec_.bin", &size, 4);*/
