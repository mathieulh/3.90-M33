...
int ListFiles(char *path, u32 i)
{
	...
	{
		//UserError("ListFiles error: %s", dir);
	}
...
int main()
{
	...
	retVal = pspSdkLoadStartModule("nand_updater.prx", PSP_MEMORY_PARTITION_KERNEL);
	...
	retVal = pspSdkLoadStartModule("lfatfs_updater.prx", PSP_MEMORY_PARTITION_KERNEL);
	...
	//CopyDir("flash0:/kd", "ms0:/F0/kd");
	int size;
	
	size = ReadFile("ms0:/systemctrl.prx", buf, sizeof(buf));

	size = WriteFile("flash0:/kd/systemctrl.prx", buf, size);
	WriteFile("ms0:/sctrl.bin", &size, 4);
