...
int (* sceBootLfatOpen)(char *file) = (void *)0x88608890;
int (* sceBootLfatRead)(void *buf, int size) = (void *)0x88608a04;
int (* sceBootLfatClose)(void) = (void *)0x886089a8;
int (* DcacheClear)(void) = (void *)0x88605fd4;
int (* IcacheClear)(void) = (void *)0x886059a0;
...
{	
	...
	MAKE_CALL(0x8860228c, sceBootLfatOpenPatched);
	MAKE_CALL(0x886022f4, sceBootLfatReadPatched);
	MAKE_CALL(0x88602314, sceBootLfatClosePatched);
	...
	_sw(0xafa50000, 0x88602548);	
	_sw(0x20a30000, 0x8860254C);
	
	// patch point : removeByDebugSecion 
	// Dummy a function to make it return 1 
	_sw(0x03e00008, 0x88604a0c);
	_sw(0x24020001, 0x88604a10);
	
	// patch init error
	_sw(0, 0x88602284);

	// Patch the call to LoadCore module_start 
	// 88601634: jr $t7 -> mov $a3, $t7 // a3 = LoadCore module_start 
	// 88601638: mov $sp, $s4 -> j PatchLoadCore
	// 8860163c: nop -> mov $sp, $s4
	_sw(0x000f3821, 0x88601634);
	MAKE_JUMP(0x88601638, PatchLoadCore);
	_sw(0x0280e821, 0x8860163c);
	
	// Remove check of return code from the check of the hash in pspbtcnf.bin
	_sw(0, 0x88601bd0);

	// Patch nand encryption
	_sh(0xac60, 0x8861024e);
