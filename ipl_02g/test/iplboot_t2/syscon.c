...
int pspSyscon_init(void)
{
	...
	saved = (u32 *)0x040f0000;
	Sysreg_driver_Unkonow_d6628a48(0,1);

	Sysreg_driver_Unkonow_8835d1e1(0);

	//sceSysregSpiIoEnable(0)
	saved[10] = REG32(0xbc100078);
	REG32(0xbc100078) |= (0x1000000<<0);

	// init SPI
	saved[6] = REG32(0xbe580000);
	saved[7] = REG32(0xbe580004);
	saved[8] = REG32(0xbe580014);
	saved[9] = REG32(0xbe580024);
	...
	// sceGpioPortClear
	saved[4] = REG32(0xbe24000c);
	REG32(0xbe24000c) = 0x08;

	// GPIO3 OUT
	saved[5] = REG32(0xbe240000);
	REG32(0xbe240000) |= 0x08;
	// GPIO4 IN
	REG32(0xbe240000) &= ~0x10;

	saved[0] = REG32(0xbe240010);
	saved[1] = REG32(0xbe240014);
	saved[2] = REG32(0xbe240018);
	saved[3] = REG32(0xbe240024);
...
int pspSyscon_end()
{
	REG32(0xbe240010) |= 0x10;//saved[0];
	REG32(0xbe240014) |= 0x10; //  saved[1];
	REG32(0xbe240018) &= ~0x10; // saved[2];
	REG32(0xbe240024) = 0; // saved[3];
	REG32(0xbe24000c) = 0; // saved[4];
	REG32(0xbe240000) |= 0x10; // saved[5];

	REG32(0xbe580000) = 0; //saved[6];
	REG32(0xbe580004) = 0; //saved[7];
	REG32(0xbe580014) = 0; //saved[8];
	REG32(0xbe580024) = 0; //saved[9];

	Sysreg_driver_Unkonow_d6628a48(0,1);

	REG32(0xbc100078) &= ~(0x1000000<<0);
	sceSysregSpiClkDisable(0);
